...
public class WndGameInProgress extends Window {
	...
	public WndGameInProgress(final int slot){
		...
		title.label((Messages.get(this, "title", info.level, className)).toUpperCase(Locale.ENGLISH) + " (" + info.gameMode.toString() + ")");
		...
		if (info.conducts != null && info.conducts.isConductedAtAll()) {
			...
			btnConducts.icon(Icons.get(Icons.CONDUCTS_COLOR));
		...
		int atuBonus = info.atuBonus;
		if (atuBonus > 0)           statSlot( Messages.get(this, "atu"), info.atu + " + " + atuBonus );
		else if (atuBonus < 0)      statSlot( Messages.get(this, "atu"), info.atu + " - " + -atuBonus );
		else                        statSlot( Messages.get(this, "atu"), info.atu );
