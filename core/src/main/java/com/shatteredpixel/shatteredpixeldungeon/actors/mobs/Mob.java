...
public abstract class Mob extends Char {
	...
	public void hordeSpawnAttempt(Level level){
		if (!hordeSpawned && hordeException() && (Random.Int(5) == 0 || Dungeon.isChallenged(Conducts.Conduct.HORDES)) && !Dungeon.bossLevel() && alignment == Alignment.ENEMY){

			int hordeSize = Math.min(Dungeon.isChallenged(Conducts.Conduct.HORDES) ? 4 : 3, Random.IntRange(1, Dungeon.depth / (Dungeon.isChallenged(Conducts.Conduct.HORDES) ? 6 : 8)));
...
//		ignored.add(SoulFlame.class); not implemented yet
	...
	public void damage(int dmg, DamageSource src ) {
		...
		if (!isInvulnerable(src.getClass())) {
			...
			if (!(src.hasProperty(DamageProperty.DECAY))) {
				...
				if (src.hasProperty(DamageProperty.MAGICAL)) {
	...
	public void die( Object cause ) {
		...
		if (alignment == Alignment.ENEMY){
			...
			if (hero.hasTalent(Talent.UNIQUE_ATTENTION)){
				Buff.affect(hero, ArtifactRecharge.class).set(hero.pointsInTalent(Talent.UNIQUE_ATTENTION)*1.5f);
			}
    ...
    public void gauntletProcessing() {
        if (Dungeon.mode == Dungeon.GameMode.GAUNTLET){
            ...
            if (!mobsAlive && Dungeon.level.locked){
                Dungeon.level.drop(new WornKey(Dungeon.depth), Dungeon.hero.pos).sprite.drop();
	...
	public void showCurrentTarget(){
		//having absolutely no context is so fickin' awesome, guys
		if (enemy != null && enemy.sprite != null && enemy.sprite.parent != null && sprite != null && sprite.visible && enemy.sprite.visible){
