...
acuRoll *= accMulti;
	...
	public void damage(int dmg, DamageSource src ) {
		...
		if (src instanceof Char && src.hasProperty(DamageProperty.PHYSICAL) && ((Char) src).buff(Kinetic.KineticTracker.class) != null){
			int dmgToAdd = 0;
		...
		if (sprite != null) {
			...
			if (damageProperties.contains(DamageProperty.CRUMBLING))    icon = FloatingText.PHYS_DMG_NO_BLOCK;
			if (damageProperties.contains(DamageProperty.MAGICAL))      icon = FloatingText.MAGIC_DMG;
	...
	public interface MoblikeCallback {
