...
public enum Talent {
	...
	//Huntress T1
	NATURES_BOUNTY(96), SURVIVALISTS_INTUITION(97), FOLLOWUP_STRIKE(98), NATURES_AID(99), SCOUTS_BARRIER(99),
	//Huntress T2
    ...
    public static void onTalentDegraded(Hero hero, Talent talent, boolean isGone){
        ...
        if (isGone){
            ...
            if (talent == ETERNAL_FRIENDSHIP){
                Buff.detach(hero, EternalFriendshipTracker.class);
            }
            if (talent == SPIRITUAL_BARRIER){
                for (Mob mob: Dungeon.level.mobs) {
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case ADVENTURER:
				...
				break;
			case CONJURER:
				Collections.addAll(tierTalents, INSPIRING_MEAL, LIQUID_CASTING, ENERGIZED_SUPPORT, MANABURN, SOULS_BURST);
				break;
			case CLERIC:
