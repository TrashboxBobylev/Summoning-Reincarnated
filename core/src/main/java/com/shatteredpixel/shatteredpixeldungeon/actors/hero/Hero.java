...
public class Hero extends Char {
	...
	public int ATU() {
		...
		if (Dungeon.level != null) {
			Wizard.ATUBoostBuff wizardBoost = buff(Wizard.ATUBoostBuff.class);
			if (wizardBoost != null){
				attunementBonus += wizardBoost.query();
			}
		}
	...
	public boolean shoot( Char enemy, MissileWeapon wep ) {
		...
		if (hit && (heroClass == HeroClass.DUELIST || Dungeon.isChallenged(Conducts.Conduct.EVERYTHING)) && wasEnemy){
			Buff.affect( this, Sai.ComboStrikeTracker.class).addHit(enemy);
		}
	...
	public boolean act() {
		...
		if (heroClass == HeroClass.CONJURER) {
			if (HP <= HT / 2) {
				Buff.affect(this, SoulSparking.class);
			}
			else if (buff(SoulSparking.class) != null) {
				buff(SoulSparking.class).detach();
			}
		}
...
@Override
	...
	public void resurrect() {
		...
		for (Item i : belongings){
			...
			} else if (i instanceof ChargingItem && i.keptThroughLostInventory()){
				if (holster != null && holster.contains(i)){
					((ChargingItem) i).charge(this, MagicalHolster.HOLSTER_SCALE_FACTOR);
				} else {
					((ChargingItem) i).charge(this);
				}
