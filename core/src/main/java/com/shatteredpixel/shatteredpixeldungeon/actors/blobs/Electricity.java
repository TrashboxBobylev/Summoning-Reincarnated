...
public class Electricity extends Blob implements DamageSource {
			...
			for (int j = area.top-1; j <= area.bottom; j++) {
				...
				if (cur[cell] > 0) {
					...
					if (h != null){
						...
						} else if (toShock instanceof ElectricalExplosive){
							((ElectricalExplosive) toShock).gainCharge(0.333f);
						}
					}
