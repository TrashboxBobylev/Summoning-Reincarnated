...
public class WandOfFireblast extends DamageWand {
	...
	public void onHit(Char attacker, Char defender, int damage) {
		...
		float procChance = 0;
        PathFinder.buildDistanceMap( defender.pos, BArray.not( Dungeon.level.solid, null ), range );
        for (int i = 0; i < PathFinder.distance.length; i++) {
            if (PathFinder.distance[i] < Integer.MAX_VALUE) {

                //+25% proc chance per burning char within (1+rank*2)x(1+rank*2) of target
                // this includes the attacker and defender
                if (Actor.findChar(i) != null
                        && Actor.findChar(i).buff(Burning.class) != null) {
                    procChance += 0.25f;

                    //otherwise +5% proc chance per burning tile within (1+rank*2)x(1+rank*2) of target
                } else if (Fire.volumeAt(i, Fire.class) > 0) {
                    procChance += 0.05f;
                }
            }
		}
			...
			if (fire != null) {
                for (int i = 0; i < PathFinder.distance.length; i++) {
                    if (PathFinder.distance[i] < Integer.MAX_VALUE) {
                        ...
                        if (Fire.volumeAt(i, Fire.class) > 0) {
                        ...
                        if (ch != null) {
                            ...
                            if (ch.alignment == Char.Alignment.ENEMY) {
                                ...
                                ch.damage(Math.round(powerMulti * Hero.heroDamageIntRange((int) (2 + 2 * power()), (int) (8 + 4 * power()))), this);
                            }
                        }
                    }
                }
...
//	}
	...
	public EnumSet<DamageProperty> initDmgProperties() {
		return EnumSet.of(DamageProperty.ENRAGES_GREATER_FIRE, DamageProperty.FIRE);
	}
