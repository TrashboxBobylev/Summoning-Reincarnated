...
public class RingOfSharpshooting extends Ring {
		...
		if (isIdentified()){
			...
			if (isEquipped(Dungeon.hero) && soloBuffedBonus() != combinedBuffedBonus(Dungeon.hero)){
				info += "\n\n" + Messages.get(this, "combined_stats",
						combinedBuffedBonus(Dungeon.hero), Messages.decimalFormat("#.##", 100f * (Math.pow(1.175, combinedBonus(Dungeon.hero)) - 1f)));
			}
			return info;
		} else {
			return Messages.get(this, "typical_stats", 1, Messages.decimalFormat("#.##", 17.5f));
		}
	}
	...
	public String upgradeStat2(int level) {
		if (cursed && cursedKnown) level = Math.min(-1, level-3);
		return Messages.decimalFormat("#.##", 100f * (Math.pow(1.175, level+1)-1f)) + "%";
	}
	...
	public static float durabilityMultiplier( Char target ){
		return (float)(Math.pow(1.15, getBonus(target, Aim.class)));
	}
