		...
		do {
			switch (Random.Int(4)){
				...
				case 2:
					...
					break;
				case 3:
					mainLoot = Generator.random(Generator.Category.STAFF);
					break;
			}
		} while ( mainLoot == null || Challenges.isItemBlocked(mainLoot));
		level.drop(Challenges.process(mainLoot), remains).setHauntedIfCursed().type = Heap.Type.SKELETON;
		
		int n = Random.IntRange( 1, 2 );
		for (int i=0; i < n; i++) {
			level.drop( Challenges.process(prize( level )), remains ).setHauntedIfCursed();
		}
