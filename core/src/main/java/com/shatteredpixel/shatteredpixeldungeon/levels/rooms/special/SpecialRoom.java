...
public abstract class SpecialRoom extends Room {
	...
	public static void initForFloor(){
		...
		//laboratory and attunement rooms spawn on floor 3 or 4 each chapter
	...
	private static void useType( Class<?extends Room> type ) {
		if (Dungeon.mode == Dungeon.GameMode.CHAOS &&
				(type != AttunementPotRoom.class && type != LaboratoryRoom.class && type != PitRoom.class && type != MysteryMerchantRoom.class)){
			//deck? what's that
		} else {
	...
	public static SpecialRoom createRoom(){
		...
		} else if (floorSpecials.contains(AttunementPotRoom.class)) {

			useType(AttunementPotRoom.class);
			return new AttunementPotRoom();
